...
const QuizzesTableRow = ({ quiz, number }) => {
  const { id, question, video_title } = quiz || {};
  const [deleteQuiz, { isLoading }] = useDeleteQuizMutation();

  //delte quiz
  const handleDeleteQuiz = () => {
    const confirm = window.confirm("Do you really want to delete this quiz?");
    confirm && deleteQuiz(id);
  };
  return (
    <tr>
      ...
      <td className="table-td flex gap-x-2 justify-center">
        <button disabled={isLoading} onClick={handleDeleteQuiz}>
          <svg
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
        ...
        <Link to={`/admin/quizzes/edit/${id}`} state={{ title: "Edit Quiz" }}>
          <svg
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
